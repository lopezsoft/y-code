...
export class CustomersComponent extends JqxCustomGridComponent implements OnInit, AfterViewInit{
  ...
  ngAfterViewInit(): void {
    const ts  = this;
    ts.crudApi        = {
      create  : '/persons/customers/create',
      read    : '/persons/customers/read',
      update  : '/persons/customers/update/',
      delete  : '/persons/customers/delete/'
    };
    ts.showActions    = true;
  ...
  createData(): void {
    const ts    = this;
    super.createData();
    ts.goRoute('sales/customers/create');
  }

  editData(data: any): void {
    super.editData(data);
    this.goRoute(`sales/customers/edit/${data.id}`);
  }
