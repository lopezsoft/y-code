...
export class InvoiceBillComponent extends FormComponent implements OnInit, AfterViewInit {
  ...
  onCellEndEdit(e: any) {
    if(e.args.dataField != "reason"){
      ...
      setTimeout(() => {
        ...
        this.onLineTotal(row, e.args.rowindex);
      }, 50);
    }
  }
  ...
  onDelete() {
    ...
    if( ts.rowIndex >= 0){
        ...
        const tRows = grid.getrows();
        grid.clear();
        tRows.map((data, i) => {
          grid.addrow(i, data);
          setTimeout(() => {
            ts.onLineTotal(data, i);
          },10);
        });
      }
			ts.onRowUnselect(null);
			setTimeout(() => {
  ...
  onLineTotal(ele: Items, i: number) {
    ...
    const tax       = parseFloat(taxe ? taxe.decimal_rate.toString() : '1');
	...
	cellClick(event: any, content: any): void
  {
    ...
    }else if(data.datafield === "#edit_#") {
      ts.onEditInvoice(row);
    }
  }
  ...
  onChangeProducts(e: number) {
    const ts  = this;
    const it  = ts.itemsResult.find(item => item.id == e);
    ts.itemSelect = it;
    ts.onAddSales();
  }
