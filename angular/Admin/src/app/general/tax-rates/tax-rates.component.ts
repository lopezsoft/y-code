...
export class TaxRatesComponent extends JqxCustomGridComponent implements OnInit, AfterViewInit{
		...
		setTimeout(() => {
			...
			ts.sourceColumns =
				[
					...
					{ text: lang.instant('taxes.rate.is_exempt'), align: 'center', datafield: 'is_exempt', columntype: 'checkbox', 
						threestatecheckbox: true, width: 120 },
  ...
  createData(): void {
    const ts = this;
    super.createData();
    ts.goRoute('general/taxrates/create');
  }

  editData(data: any): void {
    super.editData(data);
    this.goRoute(`general/taxrates/edit/${data.id}`);
  }
}
