...
class TaxesController extends Controller
{
    public function create(Request $request)
    {
        ...
        try {
            ...
            if($company){
                $data       = [
                    'name_taxe'         => $request->name_taxe,
                    'description'     => $request->description,
                    'state'       => 1,
                ];
                $table      = $company->database_name.".tax_group";
    ...
    public function select(Request $request)
    {
        ...
        if($company){
            ...
            $sqlStatement =
                "SELECT a.*
                FROM {$table}tax_group a";
            $sqlStatementCount =
                "SELECT count(a.id) as total FROM {$table}tax_group a ";

            $searchFields = ['a.name_taxe'];
